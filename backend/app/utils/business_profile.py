...
def _optional_non_negative_int(payload: Dict[str, Any], field: str):
    value = payload.get(field)
    if value is None or str(value).strip() == "":
        return None
    try:
...
def _optional_positive_int(payload: Dict[str, Any], field: str):
    value = payload.get(field)
    if value is None or str(value).strip() == "":
        return None
    try:
...
def validate_and_normalize_business_profile(profile_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    normalized = {
        ...
        "business_name": _optional_string(profile_data, "business_name"),
        "phone": _optional_string(profile_data, "phone"),
        "city": _optional_string(profile_data, "city"),
    ...
    if business_type == "HOTEL":
        normalized["details"] = {
            "total_rooms": _optional_positive_int(profile_data, "total_rooms"),
        ...
        normalized["details"] = {
            "cuisine": _optional_string(profile_data, "cuisine"),
            "opening_hours": _optional_string(profile_data, "opening_hours"),
            "seating_capacity": _optional_positive_int(profile_data, "seating_capacity"),
            "image_urls": _to_string_list(profile_data.get("image_urls"), "image_urls"),
        }

    elif business_type == "CAB_DRIVER":
        normalized["details"] = {
            "driver_name": _optional_string(profile_data, "driver_name"),
            "vehicle_type": _optional_string(profile_data, "vehicle_type"),
            "vehicle_number": _optional_string(profile_data, "vehicle_number"),
            "license_number": _optional_string(profile_data, "license_number"),
            "service_area": _optional_string(profile_data, "service_area"),
        }

    elif business_type == "TOURIST_GUIDE_SERVICE":
        services = _to_string_list(profile_data.get("service_categories"), "service_categories")
        normalized["details"] = {
            "guide_name": _optional_string(profile_data, "guide_name"),
            "personal_bio": _optional_string(profile_data, "personal_bio"),
            "years_experience": _optional_non_negative_int(profile_data, "years_experience"),
