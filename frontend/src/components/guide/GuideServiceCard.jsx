import { Clock3, IndianRupee, MapPin, Pencil, Trash2, Users } from 'lucide-react';
    ...
    <Card className="space-y-3">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="text-right shrink-0">
          ...
          <p className="text-[12px] text-text-secondary">{prettyLabel(service.price_unit || 'PER_PERSON')}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-[12px]">
        <div className="rounded-lg border border-border bg-surface-sunken/40 p-2">
          ...
          <p className="text-ink font-medium mt-1">{service.location || '-'}</p>
        </div>
        <div className="rounded-lg border border-border bg-surface-sunken/40 p-2">
          <span className="inline-flex items-center gap-1 text-text-secondary">
          ...
          <p className="text-ink font-medium mt-1">{service.duration_hours || '-'}h</p>
        </div>
        <div className="rounded-lg border border-border bg-surface-sunken/40 p-2">
          <span className="inline-flex items-center gap-1 text-text-secondary">
          ...
          <p className="text-ink font-medium mt-1">{service.max_group_size || '-'}</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-1.5">
        ...
        {(service.category || []).slice(0, 4).map((item) => (
          <span key={`${service.id}-${item}`} className="px-2 py-1 rounded-full bg-surface-sunken text-text-secondary text-[11px]">
            {item}
          </span>
        ))}
      </div>
      ...
      <div className="flex items-center gap-2 pt-1">
        <Button variant="secondary" icon={Pencil} onClick={() => onEdit(service)}>
          Edit
        </Button>
        <Button variant="danger" icon={Trash2} onClick={() => onDelete(service.id)}>
          Delete
        </Button>
      </div>
    </Card>
