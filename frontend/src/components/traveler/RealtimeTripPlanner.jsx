...
export default function RealtimeTripPlanner() {
  ...
  const startPlanner = async (event) => {
    ...
    try {
      ...
      setError(
        err?.response?.data?.message || "Unable to start planner session.",
      );
    ...
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-surface-sunken border border-border relative">
          <img
            src="/Logo-removedbg.png"
            alt="TripAllied"
            className="h-4 w-4 object-contain"
          />
          <span className="text-[10px] font-bold text-ink tracking-wider uppercase">
            TripAllied AI
          </span>
          {connected && (
            <span
              className="absolute -top-1 -right-1 h-2 w-2 rounded-full bg-success animate-pulse-live border-2 border-white"
      ...
      >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <label className="space-y-1.5">
            <span className="text-label-md text-ink flex items-center gap-1">
          ...
          <label className="space-y-1.5">
            ...
            <input
              ...
              onChange={(e) =>
                setForm((prev) => ({ ...prev, trip_days: e.target.value }))
              }
              className="w-full px-3 py-2 rounded-lg border border-border text-[14px] text-ink"
            />
          </label>
          <label className="space-y-1.5">
            ...
            <input
              ...
              onChange={(e) =>
                setForm((prev) => ({ ...prev, travelers: e.target.value }))
              }
              className="w-full px-3 py-2 rounded-lg border border-border text-[14px] text-ink"
            />
          </label>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <label className="space-y-1.5">
            ...
            <input
              ...
              onChange={(e) =>
                setForm((prev) => ({ ...prev, interests_text: e.target.value }))
              }
              className="w-full px-3 py-2 rounded-lg border border-border text-[14px] text-ink"
        ...
        <label className="space-y-1.5 block">
          ...
          <textarea
            ...
            className="w-full px-3 py-2 rounded-lg border border-border text-[14px] text-ink min-h-20"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-white border border-border rounded-xl p-4">
          ...
          {sortedProgress.length === 0 ? (
            <p className="text-[13px] text-text-secondary">
              No events yet. Start a planner session.
            </p>
          ) : (
            <div className="space-y-2 max-h-75 overflow-y-auto custom-scrollbar pr-1">
              {sortedProgress.map((event, idx) => (
                <div
                  key={`${event.stage}-${idx}`}
                  className="flex items-start gap-2.5 border border-border rounded-lg p-2.5"
                >
          ...
          {(sessionStatus === "RUNNING" || sessionStatus === "QUEUED") && (
            <div className="flex items-center gap-2 mt-3 text-[12px] text-primary font-medium">
        ...
        {/* Live stream token preview */}
        <div className="bg-white border border-border rounded-xl p-4 flex flex-col">
          <h3 className="text-label-lg text-ink mb-3">Live Output</h3>
          {streamText ? (
            <div className="flex-1 overflow-y-auto custom-scrollbar max-h-75">
            ...
            <div className="flex-1 flex items-center justify-center">
              <p className="text-[13px] text-text-secondary">
                {sessionStatus === "RUNNING" || sessionStatus === "QUEUED"
                  ...
                  : "Start a session to see live output."}
              </p>
            </div>
          )}
      ...
      {result && (
        <div className="bg-white border border-border rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-5">
            ...
            <h3 className="text-display-sm text-ink">Your AI Trip Plan</h3>
          </div>
          <PlannerResultView result={result} formInput={form} />
        </div>
      )}
    </div>
  );
