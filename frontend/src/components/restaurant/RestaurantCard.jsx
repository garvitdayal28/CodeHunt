...
export default function RestaurantCard({ restaurant, onViewMenu }) {
    ...
    return (
        <div className="bg-white rounded-2xl border border-border shadow-sm overflow-hidden hover:shadow-md transition-shadow group flex flex-col h-full cursor-pointer" onClick={() => onViewMenu(restaurant)}>
            {/* Image Container */}
            ...
            <div className="p-4 flex flex-col flex-1">
                ...
                <div className="flex items-start gap-1.5 mt-2 text-text-secondary">
                    ...
                    <span className="text-[13px] line-clamp-2">
                        {restaurant.address ? `${restaurant.address}, ${restaurant.location}` : restaurant.location}
                    </span>
                </div>

                <div className="mt-4 pt-4 border-t border-border flex items-end justify-between mt-auto">
                    <div>
                        <p className="text-[12px] text-text-secondary mb-0.5">Approx. Cost</p>
                        <p className="text-[15px] font-semibold text-ink">{priceText}</p>
                    </div>
                    <Button variant="secondary" size="sm" onClick={(e) => { e.stopPropagation(); onViewMenu(restaurant); }} className="px-3 shrink-0">
