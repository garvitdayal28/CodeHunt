  ...
  return (
    ...
    >
      <Card className="p-0 overflow-hidden h-full flex flex-col">
        <div className="h-44 bg-surface-sunken shrink-0">
          {image ? (
            <img
              src={image}
              alt={hotel.name}
          ...
          {(hotel.amenities || []).length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {(hotel.amenities || []).slice(0, 4).map((amenity) => (
                <span
                  key={amenity}
                  className="text-[11px] px-2 py-0.5 rounded-md bg-surface-sunken text-text-secondary"
                >
                  {amenity}
                </span>
              ))}
          ...
          <div className="flex items-center justify-between border-t border-border pt-3 mt-auto">
            ...
            <div className="text-right">
              <p className="text-[12px] text-text-secondary inline-flex items-center gap-1">
                ...
                {hotel.total_available_rooms ?? 0} available
              </p>
              <Button
                size="sm"
                className="mt-2"
